...
const AddRecipient: React.FC<AddRecipientProps> = ({ userId, recipientCount = 0, canAccessFeatures }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background-dark">
      <header className="px-4 py-6 border-b border-white/[0.06] flex items-center gap-4 justify-between">
        ...
        <h1 className="text-lg font-semibold tracking-tight flex-1 text-center">Add Recipient</h1>
      ...
      <main className="p-6 space-y-6">
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="size-24 rounded-full bg-surface-darker border-2 border-white/10 flex items-center justify-center overflow-hidden">
            {formData.avatarUrl ? (
              <img src={formData.avatarUrl} className="h-full w-full object-cover" alt={formData.name} />
            ) : (
              <span className="text-3xl font-semibold text-primary uppercase">{getInitials(formData.name)}</span>
            )}
          </div>
          <div className="flex gap-2">
            <input
            ...
            <button
              ...
              className="h-11 px-5 bg-primary/10 border border-primary/20 text-primary rounded-xl flex items-center justify-center gap-2 hover:bg-primary/20 transition-colors font-semibold text-xs"
            >
            ...
            {formData.avatarUrl && (
              <button
                ...
                className="h-11 px-5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl flex items-center justify-center gap-2 hover:bg-red-500/20 transition-colors font-semibold text-xs"
              >
        ...
        {isAtCap ? (
          <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4">
            ...
            <button
              ...
              className="w-full h-12 bg-primary text-white font-semibold rounded-xl hover:bg-blue-600 transition-all"
            >
              Back to Recipients
            </button>
          </div>
          ...
          <button
            ...
            className="w-full h-14 bg-primary text-white font-semibold rounded-2xl shadow-xl shadow-primary/20 hover:bg-blue-600 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Add Recipient
          </button>
        )}
